...
import app.interfold.app.utils.SETTINGS_KEY
import app.interfold.app.utils.javaPreferences
import app.interfold.app.utils.platformUtilities
import app.interfold.app.utils.runOnUiThread
...
fun main(args: Array<String>) {
  ...
  // To disable URL registration in dev mode, set INTERFOLD_SKIP_URL_REGISTRATION=true
  val skipUrlRegistration = System.getenv("INTERFOLD_SKIP_URL_REGISTRATION")?.toBoolean() ?: false
  ...
  if (!skipUrlRegistration) {
    try {
      if (UrlSchemeHandler.isDevMode()) {
        ...
        println("To skip URL registration, set INTERFOLD_SKIP_URL_REGISTRATION=true")
      }
    ...
    println("URL scheme registration skipped (INTERFOLD_SKIP_URL_REGISTRATION=true)")
  ...
  if (args.isNotEmpty()) {
    ...
    if (possibleUrl.startsWith("interfold://")) {
