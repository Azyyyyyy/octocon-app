...
object UrlSchemeHandler {
    private const val URL_SCHEME = "interfold"

    /**
     * Registers the interfold:// URL scheme with the operating system.
    ...
    private fun registerWindowsUrlScheme() {
        try {
            ...
            val regCommands = listOf(
                """reg add "HKEY_CURRENT_USER\Software\Classes\$URL_SCHEME" /ve /d "URL:Interfold Protocol" /f""",
    ...
    private fun registerLinuxUrlScheme() {
        try {
            ...
            val desktopFile = File("$homeDir/.local/share/applications/interfold-url-handler.desktop")
            ...
            Runtime.getRuntime().exec(arrayOf(
                "xdg-mime", "default", "interfold-url-handler.desktop", "x-scheme-handler/$URL_SCHEME"
            )).waitFor()
    ...
    private fun createDevelopmentLauncher(): String {
        ...
        val launcherFile = if (isWindows) {
            File(tmpDir, "interfold-dev-launcher.bat")
        } else {
            File(tmpDir, "interfold-dev-launcher.sh")
        }
    ...
    fun isDevMode(): Boolean {
        return app.interfold.app.utils.BuildConfig.isDebug()
    }

    /**
     * Parses a deep link URL and extracts relevant information
     * Format: interfold://deep/auth/token?token=xxx&id=yyy
     */
