@file:Suppress("LocalVariableName", "NOTHING_TO_INLINE", "unused")

package app.interfold.app.utils.crypto
...
class AES(val keyWords: IntArray) : Cipher {
  ...
  companion object {
    ...
    fun encryptAesEcb(data: ByteArray, key: ByteArray, padding: Padding): ByteArray =
      AES(SHA256().digest(key))[CipherMode.ECB, padding].encrypt(data)

    fun decryptAesEcb(data: ByteArray, key: ByteArray, padding: Padding): ByteArray =
      AES(SHA256().digest(key))[CipherMode.ECB, padding].decrypt(data)
  }
}
