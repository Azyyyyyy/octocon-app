...
enum InterfoldWidgetError {
  case PIN_IS_PROTECTED
  case NOT_LOGGED_IN
  case NETWORK_ERROR
}

struct InterfoldWidgetData {
  let frontingData: [AlterFrontDataWithImage]
}

struct InterfoldWidgetEntry: TimelineEntry {
  let date: Date
  let state: State

  enum State {
    case error(InterfoldWidgetError)
    case data(InterfoldWidgetData)
  }
}

func generateDummyEntry() -> InterfoldWidgetEntry {
  return InterfoldWidgetEntry(date: .now, state: .data(InterfoldWidgetData(
    frontingData: [
