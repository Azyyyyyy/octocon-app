...
struct InterfoldWidgetProvider: TimelineProvider {
  ...
  init() {
    ...
    self.groupDefaults = UserDefaults(suiteName: "group.app.interfold.Interfold")!
    self.imageManager = SDWebImageManager.shared
  }
  
  func placeholder(in context: Context) -> InterfoldWidgetEntry {
    InterfoldWidgetEntry(date: .now, state: .data(InterfoldWidgetData(frontingData: [])))
  }
  
  func getSnapshot(in context: Context, completion: @escaping (InterfoldWidgetEntry) -> ()) {
    completion(generateDummyEntry())
  }
  
  func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
    
    func generateErrorTimeline(error: InterfoldWidgetError) -> Timeline<Entry> {
      Timeline(entries: [
        InterfoldWidgetEntry(date: .now, state: .error(error))
      ] as [InterfoldWidgetEntry], policy: .atEnd)
    }
    let keychain = KeychainSwift()
    keychain.accessGroup = "AVJM9TZ9VF.app.interfold.InterfoldApp.Keychain"
    ...
    Task {
      let headers: HTTPHeaders = [
        ...
        "User-Agent": "Interfold iOS v\(Bundle.main.releaseVersionNumber ?? "UNKNOWN") (\(Bundle.main.buildVersionNumber ?? "UNKNOWN"))"
      ]
      ...
      switch response.result {
        case .success(let frontingResponse):
          ...
          let timeline = Timeline(entries: [
            InterfoldWidgetEntry(date: .now, state: .data(InterfoldWidgetData(frontingData: dataWithImages)))
          ], policy: .atEnd)
          completion(timeline)
    ...
    func tryGetCachedEntry() async -> Entry {
      guard let cachedRawData = groupDefaults.data(forKey: "cachedFrontWidgetEntry") else {
        return InterfoldWidgetEntry(date: .now, state: .error(.NOT_LOGGED_IN))
      }
      ...
      return InterfoldWidgetEntry(date: .now, state: .data(InterfoldWidgetData(frontingData: dataWithImages)))
    }
